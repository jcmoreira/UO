...
class Connection:
    ...
    def receive(self, size=4096):
        ...
        try:
            ...
            if not data:
                err = 'Can not find the Stealth window.'
        ...
        while 1:
            ...
            if len(data) < 8:
                self._buffer += data
                break
            size, type_, length = struct.unpack_from('IHH', data, offset)
            if size > len(data):
                self._buffer += data
                break
            offset += 8
            ...
            # packet type is 3 (an event callback)
                ...
                tmp, index, count = struct.unpack_from('3B', data, offset)
                offset += 3
                # parse args
                args = []
...
class ScriptMethod:
    ...
    def __call__(self, *args):
        ...
        # form packet
        header = struct.pack('2H', self.index, len(data))
...
def get_port():
    ...
    if not winapi.SendMessage(hwnd, winapi.WM_COPYDATA, 0, copydata.pointer):
        error = 'Can not send message. ErNo: {}'.format(winapi.GetLastError())
        winapi.MessageBox(0, error.decode() if b'' == '' else error,  # py2
                           'Error'.decode() if b'' == '' else 'Error', 0)
        exit(1)
    # wait for an answer
