...
def AddToSystemJournal(*args, **kwargs):
    ...
    for arg in args:
        text += sep if text else ''  # add separator if needed
        text += str(arg)
    ...
    text += end
    while text:
        _add_to_system_journal(text[:32760])  # too long
        text = text[32760:]
...
_change_profile_ex = _ScriptMethod(366)  # ChangeProfileEx
...
_get_buff_bar_info = _ScriptMethod(353)  # GetBuffBarInfo
...
_get_skill_current_value = _ScriptMethod(355)  # GetSkillCurrentValue
...
def UseType(ObjType, Color):
    _use_type(ObjType, Color)

def UseType2(ObjType):
    _use_type(ObjType, 0xFFFF)
...
class _XmfHTMLTok:
    fmt = 7*'i' + 'ISii'  # it must be 'iiiiiiiISii' not '7iIS2i'
...
def GetGumpInfo(GumpIndex):
    ...
    for cls in elements:
        ...
        count = _ushort.from_buffer(data, offset).value
        offset += 2
        for i in range(count):
            fmt = cls.fmt
            strings = []
            # calculate a size of element
            while 'S' in fmt:
                before = _struct.calcsize(fmt[:fmt.index('S')])
                strings.append(fmt.index('S'))
                str_size = _uint.from_buffer(data, offset+before)
                fmt = fmt.replace('S', 'I{0}s'.format(str_size*2), 1)
            size = _struct.calcsize(fmt)
            # unpack element
            values = list(_struct.unpack_from(fmt, data, offset))
            offset += size
            for index in strings[::-1]:
                values[index+1] = values[index+1].decode('UTF-16le')
                del values[index]  # delete a string size
            if cls.keys is None:  # _Text
                result[cls.container].append(*values)
            else:
                result[cls.container].append(dict(zip(cls.keys, values)))
    return result
...
def EquipDressSet():
    res = []
    data = _get_dress_set()
    items = _struct.unpack_from('19I', data)
    if not any(items):
        return False
    delay = GetDressSpeed()
    for layer, item in zip(_wearable_layers, items[:-1]):
        if item:
...
_messenger_get_connected = _ScriptMethod(368)  # Messenger_GetConnected
...
def MessengerGetConnected(MesID):
    if MesID not in _messengers.keys():
        error = 'MessengerGetConnected: MesID must be "Telegram" or "Viber"'
        raise ValueError(error)
    return _messenger_get_connected(_messengers[MesID])


_messenger_set_connected = _ScriptMethod(269)  # Messenger_SetConnected
_messenger_set_connected.argtypes = [_uint,  # MesID
                                     _bool]  # Value
...
_messenger_get_token = _ScriptMethod(370)  # Messenger_GetToken
...
_messenger_set_token = _ScriptMethod(371)  # Messenger_SetToken
...
_messenger_get_name = _ScriptMethod(372)  # Messenger_GetName
...
_messenger_send_message = _ScriptMethod(373)  # Messenger_SendMessage
...
_uint_to_flags = _ScriptMethod(354)  # ConvertIntegerToFlags
...
def GetStaticTilesArray(Xmin, Ymin, Xmax, Ymax, WorldNum, TileTypes):
    if not _iterable(TileTypes):
        TileTypes = [TileTypes]
    result = []
    data = _get_statics_array(Xmin, Ymin, Xmax, Ymax, WorldNum,
                              len(TileTypes),
...
_set_open_doors = _ScriptMethod(349)  # SetMoveOpenDoor
...
_get_open_doors = _ScriptMethod(350)  # GetMoveOpenDoor
...
_set_move_through_npc = _ScriptMethod(351)  # SetMoveThroughNPC
...
_get_move_through_npc = _ScriptMethod(352)  # GetMoveThroughNPC
...
_set_move_through_corner = _ScriptMethod(362)  # SetMoveThroughCorner
...
_get_move_through_corner = _ScriptMethod(363)  # GetMoveThroughCorner
...
_set_move_heuristic_mult = _ScriptMethod(364)  # SetMoveHeuristicMult
...
_get_move_heuristic_mult = _ScriptMethod(365)  # GetMoveHeuristicMult
...
_set_move_check_stamina = _ScriptMethod(366)  # SetMoveCheckStamina
...
_get_move_check_stamina = _ScriptMethod(367)  # GetMoveCheckStamina
...
_set_move_turn_cost = _ScriptMethod(368)  # SetMoveTurnCost
...
_get_move_turn_cost = _ScriptMethod(369)  # GetMoveTurnCost
...
_set_move_between_two_corners = _ScriptMethod(370)  # SetMoveBetweenTwoCorners
...
_get_move_between_two_corners = _ScriptMethod(371)  # GetMoveBetweenTwoCorners
