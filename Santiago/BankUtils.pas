...
implementation
  ...
  procedure RestockItemFromBankContainer(itemType: Cardinal; neededQuantity: Integer; containerType: Cardinal);
  var
    amountToMove : Integer;
    storageContainer: Cardinal;
  begin
    while True do
    begin
      FindType(itemType, BackPack);
      amountToMove := neededQuantity - FindFullQuantity;
      if amountToMove = 0 then break;

      if not BankIsOpen() then OpenBank();
      storageContainer := FindType(containerType, Bank());
      UseObject(storageContainer);
      Wait(3000);
      FindType(itemType, storageContainer);
      if FindFullQuantity = 0 then break;

      MoveItem(FindItem, Min(50, amountToMove), BackPack, 0, 0, 0);
      Wait(1000);
    end;
  end;
